...
export function OrganizationSwitchButton({ 
  ...
  const handleSwitchOrganization = async (org: Organization) => {
    ...
    try {
      ...
      // Try the new switch_organization function first, fallback to manual update
      try {
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-80">
        ...
        {organizations.slice(0, showAllOrgs ? organizations.length : 5).map((org) => {
        ...
        {organizations.length > 5 && !showAllOrgs && (
          <DropdownMenuItem onClick={() => setShowAllOrgs(true)}>
            <div className="flex items-center gap-2 text-muted-foreground">
              <Users className="w-4 h-4" />
              <span>Show all {organizations.length} organizations</span>
            </div>
          </DropdownMenuItem>
        )}
        
        {showAllOrgs && organizations.length > 5 && (
          <DropdownMenuItem onClick={() => setShowAllOrgs(false)}>
            <div className="flex items-center gap-2 text-muted-foreground">
              <Users className="w-4 h-4" />
              <span>Show less</span>
            </div>
