...
export default function SupervisorTeams() {
  ...
  const fetchTeams = async () => {
    try {
      ...
      // Fetch teams where current supervisor is the supervisor
  ...
  const handleCreateTeam = async () => {
    try {
      ...
      // Add the new team's member count (1 for the supervisor)
      if (data?.id) {
    ...
    try {
      ...
      if (error) {
        if (error.code === "23505") {
          toast({
            title: "Error",
            description: "This employee is already a member of this team",
            variant: "destructive",
          });
        } else {
          throw error;
      ...
      // Update member count
      if (teamMemberCounts[selectedTeam.id] !== undefined) {
        setTeamMemberCounts(prev => ({
          ...prev,
  ...
  const handleRemoveMember = async (memberId: string, teamId: string) => {
    try {
      ...
      if (teamMemberCounts[teamId] !== undefined) {
        setTeamMemberCounts(prev => ({
          ...prev,
          [teamId]: Math.max((prev[teamId] || 1) - 1, 0)
        }));
      }
    } catch (error: any) {
      toast({
                  ...
                  <div className="space-y-2">
                    {teamMembers.length === 0 ? (
                      <p className="text-sm text-muted-foreground text-center py-4">
                        No team members yet. Invite employees to join your team.
                      </p>
                    ) : (
      ...
      <Dialog open={createDialogOpen} onOpenChange={setCreateDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Create a team and invite employees to join it
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={addMemberDialogOpen} onOpenChange={setAddMemberDialogOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleAddMember}>Add Employee</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
